...
import org.fao.geonet.transformer.SchemaLocalizations;
import org.fao.geonet.transformer.TransformerConstants;
...
public class Functions extends SchemaLocalizations {
    ...
    private final TransformerParams fparams;
    private SchemaPlugin schemaPlugin;

    public Functions(TransformerParams fparams, Environment env) throws Exception {
    ...
    private String translateFromSchema(Path schemaDir, String key, String file) throws Exception {
        final Path formatterDir = schemaDir.resolve(TransformerConstants.SCHEMA_PLUGIN_FORMATTER_DIR);
    ...
    private Element getFormatterTranslations(Path dir) throws Exception {
       return fparams.context.getBean(TransformerResources.class).getPluginLocResources(dir);
    }
