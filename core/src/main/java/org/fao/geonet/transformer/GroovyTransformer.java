...
public class GroovyTransformer {
    ...
    public Transformer findTransformer(final TransformerParams fparams) throws ExecutionException {
    ...
    public String format(TransformerParams fparams) throws Exception {
    ...
    private synchronized Transformer createTransformer(TransformerParams fparams) throws Exception {
        Transformer transformer = null;
        if (fparams.formatDir != null) {
            transformer = this.transformers.getIfPresent(fparams.formatDir);
        } else {
            transformer = this.transformers.getIfPresent(fparams.viewFile);
        }

        if (fparams.isDevMode() || transformer == null) {
            final Path baseShared = this.dataDirectory.getFormatterDir().resolve(TransformerConstants.GROOVY_SCRIPT_ROOT);
            final Path schemaFormatterDir = getSchemaPluginFormatterDir(fparams.schema);
            final Path schemaShared = schemaFormatterDir.resolve(TransformerConstants.GROOVY_SCRIPT_ROOT);
            ...
            if (fparams.formatDir == null) {
                transformer = new Transformer(handlers, functions, env, fparams.viewFile.getParent().toAbsolutePath());
                this.transformers.put(fparams.viewFile, transformer);
            } else {
    ...
    private Path getSchemaPluginFormatterDir(String schema) {
        return this.schemaManager.getSchemaDir(schema).resolve(TransformerConstants.SCHEMA_PLUGIN_FORMATTER_DIR);
    }
    ...
    private GroovyClassLoader getParentClassLoader(TransformerParams fparams, String schema, Path baseShared, Path schemaShared) throws IOException,
        ...
        if (fparams.isDevMode() || cl == null) {
            ...
            if (dependOnSchema != null) {
                Path dependent = getSchemaPluginFormatterDir(dependOnSchema).resolve(TransformerConstants.GROOVY_SCRIPT_ROOT);
