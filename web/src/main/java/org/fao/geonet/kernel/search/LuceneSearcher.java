	...
	private void computeQuery(ServiceContext srvContext, Element request, ServiceConfig config) throws Exception {
		...
		if (srvContext != null) {
            ...
            if (_styleSheetName.equals(Geonet.File.SEARCH_Z3950_SERVER)) {
				...
				_query = LuceneSearcher.makeLocalisedQuery(xmlQuery, SearchManager.getAnalyzer(_language, true), _tokenizedFieldSet, _luceneConfig.getNumericFieldsMap(), _language, requestedLanguageOnly);
			} 
            else {
                ...
                _query = new LuceneQueryBuilder(_tokenizedFieldSet, _luceneConfig.getNumericFieldsMap(), SearchManager.getAnalyzer(_language, true), _language).build(luceneQueryInput);
                if(Log.isDebugEnabled(Geonet.SEARCH_ENGINE))
