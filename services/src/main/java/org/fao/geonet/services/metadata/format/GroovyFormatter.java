...
public class GroovyFormatter implements FormatterImpl {
    ...
    public String format(FormatterParams fparams) throws Exception {
        ...
        try {
            final List<Namespace> namespaces = getSchemaManager(fparams).getSchema(fparams.schema).getNamespaces();
    ...
    protected SchemaManager getSchemaManager(FormatterParams fparams) {
        return fparams.context.getBean(SchemaManager.class);
    }
    ...
    private synchronized Transformer createTransformer(FormatterParams fparams) throws Exception {
        ...
        if (fparams.isDevMode() || transformer == null) {
            GeonetworkDataDirectory dataDirectory = fparams.context.getBean(GeonetworkDataDirectory.class);
            final Path baseShared = dataDirectory.getFormatterDir().resolve(GROOVY_SCRIPT_ROOT);
            ...
            final TemplateCache templateCache = fparams.context.getBean(TemplateCache.class);
            Handlers handlers = new Handlers(fparams, schemaShared.getParent(), baseShared.getParent(), templateCache);
    ...
    private Path getSchemaPluginFormatterDir(FormatterParams fparams, String schema) {
        return getSchemaManager(fparams).getSchemaDir(schema).resolve(SCHEMA_PLUGIN_FORMATTER_DIR);
    }
